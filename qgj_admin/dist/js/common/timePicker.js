...
//获取营业时间的值
function getTimePickerVal() {
    var timePickerArr = [];
        $(".time-picker-item").each(function () {
            //有未确认的项则不存值
            if(!$(this).hasClass("js-time-area-editing")){
                var week = [],timePickerObj = {};  //清除数据

                $(this).find(".js-week-item").each(function () {
                    if($(this).hasClass("active")){
                        week.push($(this).find("input").val());
                    }
                });
                timePickerObj.week = week;
                timePickerObj.time_area_start_hour = $(this).find("select").eq(0).find("option:selected").text()+":"+$(this).find("select").eq(1).find("option:selected").text();
                timePickerObj.time_area_end = $(this).find("select").eq(2).find("option:selected").text()+":"+$(this).find("select").eq(3).find("option:selected").text();
                timePickerObj.is_all_day = $(this).find(".js-24hours").prop("checked") ? "1" : "2";
                timePickerArr.push(timePickerObj)
            }
        });
        if(timePickerArr.length){
            $("#timeAreaHidden").val(JSON.stringify(timePickerArr)); //赋值时间段值
        }else{
            $("#timeAreaHidden").val('')
        }
}
...
(function (window, $, undefined) {
    var shophours = function (options) {
        ...
        var temp = '<div class="time-picker-item js-time-area-editing">' +
            ...
            if(opts.is24hours){
                temp += '<label class="mr20"><input type="checkbox" class="qkj-checkbox js-24hours"> 24小时营业</label>';
            }

            temp += '<div class="btn-wrap"><a href="javascript:;" class="js-sure">确认</a> | <a href="javascript:;" class="js-cancel">取消</a></div>' +
            '<div class="btn-wrap" style="display:none"><a href="javascript:;" class="js-edit">编辑</a> | <a href="javascript:;" class="js-del">删除</a></div>' +
        '</div>';
        var weekValArr = [],
            $timePickerItemAdd = $(".time-picker-item-add"),
            addText = opts.addText ? opts.addText : '增加营业时间';

        function init() {
            var timeAreaHiddenVal = $("#timeAreaHidden").val(),
                json =  '',
                len = 0, //选中周的长度
                $timeArea = $(".time-area"), $timeAreaEditing = '', $curWeek = '';

            if(timeAreaHiddenVal){ json = JSON.parse($("#timeAreaHidden").val()) };
            ...
            //选中周期等于7的时候把“新增时间段”隐藏
            if(len == 7){
                $timeArea.append('<a href="javascript:;" class="time-picker-item-add" style="display: none">'+ addText +'</a>')
            }else{
                $timeArea.append('<a href="javascript:;" class="time-picker-item-add">'+ addText +'</a>')
            }
        }
        init();  //初始化
        storeWeek(); //存储所有选中周期
        ...
        //周期选择
        $(document).on('click',".js-week-item",function () {
            if($(this).hasClass('active')){
