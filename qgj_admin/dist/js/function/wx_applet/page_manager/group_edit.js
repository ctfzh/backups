...
(function (window, $, undefined) {
        ...
        var settings = $.extend({
            ...
            data_tpl: {
                ...
                banner: [
                    ...
                    '</div>' +
                    '</div>' +
                    '{{if style!=1}}<div class="form-group">' +
                    ...
                    '<i class="icon-plus"></i> 添加广告 <div class="h6">{{if style==1}}建议尺寸 750x350 像素{{else if style==4}}建议宽度 750 像素{{else if style==2}}建议宽度 300 像素{{else if style==3}}{{if lineDisplay == 1}}<span class="mr10" >4张：建议宽度 210 像素</span>{{else if lineDisplay == 2}}<span class="mr10">5张：建议宽度 160 像素</span>{{else if lineDisplay == 3}}<div>6张：建议宽度 130 像素</div>{{/if}}{{else if style==5}}建议宽度 670 像素{{/if}}</div></a></div>' +
    ...
    tpl_groupAdd.prototype = {
        init: function (opts) {
            ...
            //获取商品分组
            $.ajax({
                url: '/wechatapp/page-set-feature/get-group.html',
                type: "GET",
                dataType: 'json',
                ...
                $('#modalNav').on('shown.bs.modal', function () {
                    ...
                    if($(".js-ul-switch li").eq(1).hasClass("active")){
                        ...
                        if(img.attr("data-value")){
                            img.attr("src",_this.settings.upload_url  + "/images/"+ _this.settings.folder +"/source/" +img.attr("data-value"))
                        }
                    }
                });
                //加载图片  弹出框“确定”按钮点击
        ...
        ajaxLoad: function (data1) {  //活动商品加载
            $.ajax({
                'type': 'get',
                'url': '/wechatapp/goods/get-activity-goods.html',
                'data': data1,
                'dataType': 'json',
