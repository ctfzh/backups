     ...
     wx.login({
        success: function (res) {
           that.setData({
              ...
              MyUtils.myconsole("获取code" + res.code);
        }
     });
     wx.hideShareMenu();
    Retry(this);
...
function phone_register(that, phone_num) {
  var data = {};
	var userlnfo = Extension.getuserlnfo();
	data['mchid'] = MySign.getMchid();
	data['sign'] = MySign.sign(data);
  data['phone'] = phone_num;
  data['openid'] = Extension.getOpenid();
   data['unionid'] = Extension.getUnionid();
	if (userlnfo.nickName) {
    //用户昵称
		data['nickname'] = userlnfo.nickName;
  }
	if (userlnfo.avatarUrl) {
    //头像
		data['headimgurl'] = userlnfo.avatarUrl;
  }
	if (userlnfo.gender) {
    //性别
