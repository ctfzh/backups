      ...
      if (res.content && res.content.length > 0) {
        ...
        that.setData({
          ...
          share_data: res.share_data,
			...
			//自定义错误提示
			Extension.custom_error(that, '2', '页面加载失败', '（商户暂未设置页面）', '2');
      }
  ...
  MyRequest.request_data(
    ...
    function (res) {
      if (res) {
        ...
        MyUtils.myconsole('获取卡券数据失败');
        Extension.show_top_msg(that, '卡券数据不存在');
      }

    },
    function (res) {
      Extension.show_top_msg(that, res ? res : '卡券数据加载失败');
	  },
	  function (res) {
		  MyUtils.myconsole("请求优惠券领取的数据：");
		  MyUtils.myconsole(res);
		  //关闭加载中动画
		  wx.hideLoading()
	  })
