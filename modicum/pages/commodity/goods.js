...
function sku_item(that) {
  ...
  if (batching) {
	...
	var category_selected = category_selected ? category_selected : '';
	var methods_selected = methods_selected ? methods_selected : '';
	var batching_selected = batching_selected ? batching_selected : '';
	var only_practice = category_selected + methods_selected + batching_selected;
  that.setData({
	  category_selected: category_selected,
	  methods_selected: methods_selected,
	  batching_selected: batching_selected ,
	  only_practice: only_practice,
...
function scrolltxt(that) {
   ...
   if (length > marquee_box) {
      var maxscrollwidth = length + that.data.marquee_margin-40;//滚动的最大宽度，文字宽度+间距，如果需要一行文字滚完后再显示第二行可以修改marquee_margin值等于windowWidth即可
      ...
      Journal.myconsole(res);
		 ...
		 if (that.data.store.store.notice) {
			 ...
			 wx.createSelectorQuery().select('.scrolltxt').boundingClientRect(function (rect) {
				 var length = that.data.store.store.notice.length * 14;//文字长度
...
function get_sku(that, goods_id, single, existing_num) {
  ...
  Request.request_data(
    ...
    function (res) {
      if (res) {
			var price = Math.formatFloat(parseFloat(res.sku.price) + parseFloat(res.sku.batching_price) + parseFloat(res.sku.methods_price), 2).toFixed(2);
        that.setData({
          sku: res,
			  price: price
        })
    ...
    function (res) {
       if (res) {
        ...
        if (that.data.function_type==1){
			  var open_status = res.order_set.open_status;
		  } else {
			  var open_status = res.takeaway_set.open_status;
        }
			 ...
			 if (open_status == 1) {
				 ...
				 if (settlement == 1) {
					 ...
					 if (token) {
						 if (that.data.function_type == 1) {
							 wx.navigateTo({
								 url: '/pages/order/place_order?store_id=' + that.data.store_id + "&function_type=" + that.data.function_type + "&store_name=" + res.store.store_name,
							 })
					 ...
					 //页面数据
					 var postage_fee_activity_condition_money = parseFloat(res.merchant_set.postage_fee_activity_condition_money);
					 var send_price = parseFloat(res.takeaway_set.send_price);
					 var min_price = parseFloat(res.takeaway_set.min_price);
					 that.setData({
						 store: res,
						 postage_fee_activity_condition_money: postage_fee_activity_condition_money,
