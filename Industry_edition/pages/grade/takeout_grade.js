...
Page({
  ...
  distribution: function (e) {
    var send_score = e.currentTarget.dataset.item;
    this.setData({
...
function getcomment_add(that) {
  ...
  if (token) {
    ...
    data['mchid'] = Sign.getMchid();
	 ...
	 data['send_score'] = that.data.send_score;
    ...
    Request.request_data(
      ...
      function (res) {
        ...
        if (res) {
          ...
          if (res.finished_time) {
             sliceTime(that, res.finished_time);
          }
        } else {
          that.setData({
            show: false,
