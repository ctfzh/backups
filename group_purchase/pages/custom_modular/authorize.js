...
// 引用日志输出
var output = require('../JS/tool/output.js')
//引入签名加密商户号
...
function get_app_code(that) {
      wx.login({
        success: function (log) {
          if (log.code) {
            output.log("获取code" + log.code);
            that.setData({
              code: log.code,
              show_loading_faill: true,
            })
          } else {
            output.log('获取用户登录状态失败！' + log.errMsg)
          }
        },
        fail: function () {
        },
